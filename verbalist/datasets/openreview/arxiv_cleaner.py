...
def _read_all_tex_contents(tex_files, parameters):
    contents = {}
    for fn in tex_files:
        contents[fn] = _read_file_content(os.path.join(parameters["input_folder"], fn))
    return contents
...
def _resize_and_copy_figure(
    ...
    if resize_image and os.path.splitext(filename)[1].lower() in [
        ...
        _resize_pdf_figure(filename, origin_folder, destination_folder, pdf_resolution)
    else:
        shutil.copy(
...
def _resize_and_copy_figures_if_referenced(parameters, contents, splits):
    ...
    pdf_resolution = collections.defaultdict(lambda: parameters["pdf_im_resolution"])
...
def _keep_only_referenced(filenames, contents, strict=False):
    ...
    """
    return [
        fn for fn in filenames if _search_reference(fn, contents, strict) is not None
    ]
...
def run_arxiv_cleaner(parameters):
    ...
    full_content = "\n".join("".join(tex_contents[fn]) for fn in splits["tex_to_copy"])
